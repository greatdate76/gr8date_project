...
class Command(BaseCommand):
    help = "Strict import: creates a profile ONLY if at least one image downloads"
        ...
        for i, row in enumerate(rows, 1):
            ...
            for pos, url in enumerate(candidates):
                try:
                    ...
                    fname = filename_from_url(url, prefix=f"{uid}_img_{pos}")
                    if primary_tuple is None:
                        primary_tuple = (fname, content, url)
                    downloaded.append((fname, content, url))
                except Exception as e:
                    self.stdout.write(self.style.WARNING(f"[{i}] image fail {url} {e}"))
                    continue
