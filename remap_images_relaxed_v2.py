...
def rel_from_value(v):
    """Prefer the part after /wp-content/uploads/, else return v (sans leading slash)."""
    m = UP_RE.search(v or "")
    return m.group(1) if m else str(v).lstrip("/")

def strip_gf_face(name):
    # e.g. face_<hash>_<rest>.jpg  OR face_<rest>.jpg
    n = re.sub(r"^face_[^_]+_(.+)$", r"\1", name)
    n = re.sub(r"^face_(.+)$", r"\1", n)
    return n

def find_unique_by_basename(root: Path, basename: str):
    hits = list(root.rglob(basename))
    return hits[0] if len(hits)==1 else None

def choose_roots(uploads_root: Path):
    # Auto-detect if there's an embedded wp-content/uploads within the given root
    alt = uploads_root / "wp-content" / "uploads"
    roots = []
    if alt.exists():
        roots.append(alt)
...
def process_field(raw, roots):
    rels = []; missing=[]
    for p in split_multi(raw):
        p = fix_url(p)
        rel = rel_from_value(p)              # e.g. "gravity_forms/19-.../2022/07/file.jpg" OR "2022/07/file.jpg"
        tried = 0
        found = False
        for base in roots:
            tried += 1
            cand = (base / rel)
            if cand.exists():
                rels.append(str(cand.relative_to(base))); found=True; break
            # try if value itself still contains 'wp-content/uploads/...'
            if rel.lower().startswith("wp-content/uploads/"):
                cand2 = base / rel.split("/",2)[2]  # drop leading 'wp-content/uploads/'
                if cand2.exists():
                    rels.append(str(cand2.relative_to(base))); found=True; break
        if found:
            continue
        # unique-basename fallback
        base_name = strip_gf_face(Path(rel).name)
        for base in roots:
            hit = find_unique_by_basename(base, base_name) if base_name else None
            if hit and hit.exists():
                rels.append(str(hit.relative_to(base))); found=True; break
        if not found:
            missing.append(p)
    # de-dup keep order
    seen=set(); out=[]
...
def main():
    if len(sys.argv)<5:
        print("Usage:\n  python remap_images_relaxed_v2.py <INPUT_CSV> <OUTPUT_CSV> <UPLOADS_ROOT> <REPORT_DIR>")
        sys.exit(1)
    ...
    out_rows=[]; miss=[]; sample_logged=0
    for row in rows:
        uid = (row.get(uid_col) or "").strip()

        def mapcol(col):
            if not col or col not in row: return []
            rels, missing = process_field(row[col], roots)
    ...
    print("Wrote:", out_csv)
    print("Missing (relaxed):", (repdir/"remap_relaxed_missing.csv"), "rows:", len(miss))
